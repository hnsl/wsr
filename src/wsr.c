...
static fstr_mem_t* serialize_set_cookie(wsr_set_cookie_t set_cookie) {
    ...
    if (delete_cookie || set_cookie.expires > 0) {
        ...
        list_push_end(header_parts, fstr_t, fss(rio_clock_to_rfc1123(rio_epoch_to_clock_time(expires))));
    }
...
static wss_cb_arg_t http_session(rio_t* client_h, wsr_cfg_t cfg, rio_in_addr4_t local_addr, rio_in_addr4_t remote_addr, void* conn_data) {
    ...
    for (;;) sub_heap {
        ...
        sub_heap {
            ...
            list_push_end(raw_headers, fstr_t, concs("date: ", fss(rio_clock_to_rfc1123(rio_clock_time_now()))));
